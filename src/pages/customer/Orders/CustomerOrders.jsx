...
import { getUserOrders } from '../../../services/customerApi';
import { BASE_URL } from '../../../services/apiClient';
import CustomerOrderDetails from '../OrderDetails/CustomerOrderDetails';
...
const STATUS_ICON = {
  DRAFT: ICONS.CLOCK,
  PENDING: ICONS.CLOCK,
  CONFIRMED: ICONS.CLOCK,
  PROCESSING: ICONS.CLOCK,
  PACKED: ICONS.CLOCK,
  SHIPPED: ICONS.TRUCK,
...
// Single source of truth for tab <-> status mapping, used by both filtering and counting.
const STATUS_GROUPS = {
  ALL: null,
  PROCESSING: ['DRAFT', 'PENDING', 'CONFIRMED', 'PROCESSING', 'PACKED'],
  SHIPPED: ['SHIPPED'],
  DELIVERED: ['DELIVERED'],
...
const matchesGroup = (order, tabId) => {
  const group = STATUS_GROUPS[tabId];
  return group === null ? true : group.includes(order.status);
};
...
const CustomerOrders = () => {
  ...
  useEffect(() => {
    ...
    if (oId && orders.length > 0) {
      const matched = orders.find(
        (o) => String(o.orderId) === oId || String(o.customOrderId || '').toLowerCase() === oId.toLowerCase()
      );
  ...
  const filtered = useMemo(
    () => orders.filter((o) => matchesGroup(o, filter)),
    [orders, filter]
  );
  ...
  const tabCounts = useMemo(() => {
    const counts = {};
    TABS.forEach((tab) => {
      counts[tab.id] = orders.filter((o) => matchesGroup(o, tab.id)).length;
    });
    return counts;
  }, [orders]);
  ...
  // Keep currentPage valid if the filtered set shrinks (e.g. after a refund or switching tabs).
  useEffect(() => {
    ...
    <div className="pb-10 font-sans text-gray-800 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-3 flex items-center gap-2.5 shadow-xs w-full md:w-auto">
          ...
          <button className="flex items-center gap-1.5 px-3.5 py-2 bg-gray-50 border border-gray-150 rounded-xl text-xs font-semibold text-gray-700 hover:bg-gray-100 transition-all cursor-pointer">
            ...
            Last 6 Months
            <svg className="w-3.5 h-3.5 text-gray-400 ml-0.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 bg-white border border-gray-200 rounded-2xl shadow-xs">
          ...
          <Link
            to="/"
            className="inline-flex items-center justify-center bg-[#16A34A] hover:bg-emerald-700 text-white text-[10px] font-bold uppercase tracking-wider px-5 py-2.5 rounded-lg transition-colors cursor-pointer"
          >
            Start Shopping
          </Link>
        </div>
        ...
        <div className="space-y-4">
          {paginatedOrders.map((order) => {
            ...
            const expectedDate = ACTIVE_STATUSES.includes(order.status) && order.createdAt
              ? fmtDate(new Date(new Date(order.createdAt).getTime() + 7 * MS_PER_DAY))
              : null;
            ...
            return (
              <div
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') setExpanded(order.orderId);
                }}
              ...
              >
                ...
                <div className="flex flex-1 items-center gap-3.5 min-w-0">
                  <div className="w-14 h-14 bg-white border border-gray-150 rounded-xl flex items-center justify-center p-1 flex-shrink-0 overflow-hidden">
                    {firstItem.imagePath ? (
                      <img src={getImgUrl(firstItem.imagePath)} className="max-w-full max-h-full object-contain rounded-lg" alt={firstItem.name || 'Order item'} />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium mt-0.5">
                      {firstItem.variantLabel && (
                        <span>
                          Variant: <span className="text-slate-800 font-semibold">{firstItem.variantLabel}</span>
                        </span>
                      )}
                      {firstItem.variantLabel && firstItem.color && <span className="w-px h-2 bg-gray-300"></span>}
                      {firstItem.color && (
                        <span>
                          Color: <span className="text-slate-800 font-semibold">{firstItem.color}</span>
                        </span>
                      )}
                      {firstItem.size && (
                        <span>
                          Size: <span className="text-slate-800 font-semibold">{firstItem.size}</span>
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-gray-500 font-medium mt-0.5">
                      <span>
                        Qty: <strong className="text-slate-800 font-semibold">{firstItem.quantity || 1}</strong>
                        {order.items?.length > 1 && (
                          <span className="text-[#16A34A] ml-1 font-bold">+{order.items.length - 1} more</span>
                        )}
                      </span>
                      <span className="text-gray-300">•</span>
                      <span>
                        Order ID: <strong className="text-slate-850 font-semibold">{order.customOrderId || `#ORD-${order.orderId}`}</strong>
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full lg:w-48 flex-shrink-0 flex flex-col gap-1 border-t lg:border-t-0 lg:border-l lg:border-r border-gray-100 pt-3 lg:pt-0 lg:px-4.5 text-xs text-gray-500 font-medium">
                  ...
                  <div className="text-[10px] text-gray-400 font-medium">
                    ...
                    {expectedDate && `Expected by ${expectedDate}`}
                  </div>
                </div>
          ...
          {totalPages > 1 && (
            <div className="flex flex-col md:flex-row justify-between items-center mt-6 pt-4 border-t border-gray-150 gap-3">
              <p className="text-[12px] text-gray-500 font-semibold">
                Showing {startItem} to {endItem} of {filtered.length} orders
              </p>
              <div className="flex items-center gap-1.5">
                <button
