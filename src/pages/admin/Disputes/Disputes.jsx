...
import { getAdminRefunds, getDisputes } from '../../../services/refundApi';
import { useAdminTheme } from '../../../hooks/useAdminTheme';
import { RefreshCw, Gavel, CreditCard, ShieldAlert, AlertTriangle } from 'lucide-react';
import AdminRefundList from '../Refunds/AdminRefundList';
...
export default function AdminDisputes() {
  ...
  const stats = React.useMemo(() => {
    ...
    return { total, arbitration, processing, verification };
  }, [refunds]);
    ...
    >
      <div className="max-w-5xl mx-auto p-4 lg:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in duration-300">
          ...
          <div className={`rounded-[20px] border p-5 shadow-[0_8px_30px_rgba(0,0,0,0.03)] flex flex-col justify-between min-h-[120px] transition-all hover:shadow-[0_12px_32px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 ${themeClasses.card} ${themeClasses.border.primary}`}>
            <div className="flex items-center justify-between">
              <span className={`text-[10px] font-black uppercase tracking-widest ${themeClasses.text.tertiary}`}>Awaiting Audit</span>
            ...
            <div className="mt-4">
              <h3 className={`text-xl font-black tracking-tight leading-none ${themeClasses.text.primary}`}>{stats.verification}</h3>
              <p className={`text-[9px] font-bold uppercase tracking-wider mt-1.5 ${themeClasses.text.accent}`}>Proof under review</p>
            </div>
          </div>
        </div>
