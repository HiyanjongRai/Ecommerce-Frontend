...
import { Search, Eye, RefreshCw, Copy, Check, CreditCard, Calendar, DollarSign, X, CheckCircle, AlertTriangle, Clock } from 'lucide-react';
import AdminLayout from './AdminLayout';
import { useAdminTheme } from '../hooks/useAdminTheme';
import { getAdminPayments, getAdminPaymentEvents } from '../services/adminService';
    ...
    <AdminLayout pageTitle="Payment Management" pageSubtitle={`${payments.length} total payments processed`}>
      ...
      <div className={`px-6 py-4 border-b flex flex-wrap items-center justify-between gap-4 transition-colors ${themeClasses.bg.primary} ${themeClasses.border.primary}`}>
        <div className="flex flex-wrap items-center gap-4 flex-1">
          ...
          <div className="relative">
            ...
            <X size={12} className={`absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none transition-colors ${themeClasses.text.tertiary}`} />
          </div>
        </div>
        <button
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={9} className={`text-center py-16 font-medium transition-colors ${themeClasses.text.tertiary}`}>No payments found</td></tr>
                ) : filtered.map(p => (
                  <tr key={p.paymentId} className={`border-b transition-colors ${themeClasses.border.primary} hover:${themeClasses.bg.secondary}`}>
                    ...
                    <td className="px-4 py-3">
                      <div>
                        <p className={`font-semibold transition-colors ${themeClasses.text.primary}`}>{p.customerName || 'N/A'}</p>
                        <p className={`text-[9px] transition-colors ${themeClasses.text.tertiary}`}>{p.customerEmail}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide transition-colors ${themeClasses.bg.tertiary} ${themeClasses.text.secondary}`}>
                        {p.method}
                      </span>
                    </td>
                    <td className={`px-4 py-3 font-black transition-colors ${themeClasses.text.primary}`}>
                      {money(p.amount)}
                    </td>
                    <td className="px-4 py-3">
                      <StateBadge value={p.state} themeClasses={themeClasses} />
                    </td>
                    <td className={`px-4 py-3 text-xs font-mono transition-colors ${themeClasses.text.tertiary}`}>
                      {p.providerReferenceId ? (
      ...
      {selectedPayment && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-xs" onClick={() => setSelectedPayment(null)} />
          <div className={`relative w-full max-w-lg h-full shadow-2xl flex flex-col animate-slide-in transition-colors ${themeClasses.bg.primary}`}>
            <div className={`flex items-center justify-between px-6 py-4 border-b transition-colors ${themeClasses.border.primary}`}>
              <div>
                <h2 className={`text-base font-black transition-colors ${themeClasses.text.primary}`}>Transaction History</h2>
                <p className={`text-[11px] font-mono transition-colors ${themeClasses.text.tertiary}`}>Payment ID: #{selectedPayment.paymentId}</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className={`border rounded-2xl p-4 space-y-3 transition-colors ${themeClasses.bg.secondary} ${themeClasses.border.primary}`}>
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className={`font-bold transition-colors text-emerald-600`}>{selectedPayment.method}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                ...
                <div className={`flex justify-between items-center text-xs pt-1.5 border-t transition-colors ${themeClasses.border.primary}`}>
              ...
              {selectedPayment.transactionUuid && (
                <div className={`border rounded-2xl p-4 space-y-2 transition-colors ${themeClasses.border.primary}`}>
                  ...
                  <div className={`flex items-center justify-between p-2 rounded-xl border transition-colors ${themeClasses.bg.secondary} ${themeClasses.border.primary} text-xs font-mono`}>
              ...
              <div className="space-y-4">
                ...
                {loadingEvents ? (
                  ...
                  <div className={`text-center py-8 border border-dashed rounded-2xl text-xs transition-colors ${themeClasses.border.primary} ${themeClasses.text.tertiary}`}>
                      ...
                      return (
                        <div key={ev.id || idx} className="relative">
                          ...
                          <div className={`absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border-2 transition-colors
                            ${isComplete ? 'border-emerald-500 bg-white' : isFailed ? 'border-red-500 bg-white' : 'border-amber-500 bg-white'}`} />
                          
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <p className={`text-xs font-bold transition-colors ${themeClasses.text.primary}`}>{nice(ev.eventType)}</p>
                              <span className={`text-[9px] font-medium transition-colors ${themeClasses.text.tertiary}`}>{date(ev.createdAt || ev.timestamp)}</span>
                            </div>
            ...
            <div className={`border-t p-4 transition-colors ${themeClasses.border.primary}`}>
              <button
                onClick={() => setSelectedPayment(null)}
                className={`w-full py-2.5 rounded-lg text-xs font-bold transition-colors border ${themeClasses.border.primary} ${themeClasses.text.secondary} hover:${themeClasses.bg.tertiary}`}
              >
                Close Logs
              </button>
            </div>
